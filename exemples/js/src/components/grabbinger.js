...
AFRAME.registerComponent("grabbing", {
  ...
  init() {
    ...
    this.camflash = document.getElementById("camflash")
  ...
  grab(hand = "head") {
    ...
    let hit = ray.intersections[0]
    if (hit && hit.el.components.grabbable) {
      if (hit.el.components.injectflashlight){
        console.log("coucouc" + hit.el.getAttribute("visible"))
        hit.el.parentNode.removeChild(hit.el);
        this.camflash.setAttribute("visible",true)
        console.log("coucouc" + hit.el.getAttribute("visible"))
      }
