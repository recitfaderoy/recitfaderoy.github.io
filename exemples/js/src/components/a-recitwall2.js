...
class BSPNode {
  ...
  AFRAME.registerComponent('injectrecitwall', {
    ...
    schema: {
           ...
           'door': {'default' : "left"}, //left,center,right
    ...
    init() { 
        const  mur = new THREE.Group();     //objet 3D qui sera dans la prémitive a-recitwall  
        const data =this.data ; //attributs 
        

        /* *****définition du matériel du mur ***** */
       // const material = new THREE.MeshBasicMaterial( { color:  data.color} ); 
              const material1 = new THREE.MeshBasicMaterial( { color:  "#c2d6d6"} );
              const material2 = new THREE.MeshBasicMaterial( { color:  "blue"} );
              const material3 = new THREE.MeshBasicMaterial( {color: "brown"} );
              const material4 = new THREE.MeshBasicMaterial( {color: "red"} );
        /* *****construction de murs par somment ***** */
          /* *****sommets d'un protptype de   mur ***** */
            const vertices=[
                //devant
                { pos:[ data.longueur, 3, 0]},
              {pos:[ 0, 0, 0]},
              { pos:[ data.longueur, 0, 0]},
              ...
              { pos:[ data.longueur, 3, 0]} ,   
                ...
                { pos:[ 0, 0, 0]},
                  ...
                  { pos:[ data.longueur, 0, 0]}, 
                  { pos:[ data.longueur, 3, 0]},
                  { pos:[ data.longueur, 3, 0.2]},
                  { pos:[ data.longueur, 3, 0.2]},
                { pos:[ data.longueur, 0, 0.2]},
                { pos:[ data.longueur, 0, 0]} ,
          ...
          /* *****construction de la géométrie du mur ***** */
                  const positions = [];
                  const positions1 = [];
                  const normals = [];
                  const uvs = [];
                  ...
                  // uvs.push(...vertex.uv);
                    ...
                    Positions1= new Float32Array(positions1);
                    this.geometrya= new THREE.BufferGeometry()
                    this.geometrya.setAttribute( 'position',new THREE.BufferAttribute(Positions,3))
                    this.geometrya.computeVertexNormals();
                    this.geometrya.normalizeNormals ()
                    this.geometrya.computeBoundingBox (); 

                    const murA = new THREE.Mesh( this.geometrya, material2 );


                    this.nporte= new THREE.BufferGeometry() ;
                    this.nporte.setAttribute( 'position',new THREE.BufferAttribute(Positions,3))
                    this.nporte.computeVertexNormals();
                    this.nporte.normalizeNormals ()

                    const murB = new THREE.Mesh( this.nporte, material2 );
                   
        /* *****construction de murs par extrusion ***** */
        /* *****Début forme a estruder
        pour le mur ***** */
        const wallshape = new THREE.Shape();
            switch(data.door)
            {
        /* *****Début forme a estruder pour le mur sans porte en utilisant la longuer du mur fourni en paramètre***** */      
                  case false :
                    wallshape.moveTo(0, 0);
                    wallshape.lineTo(0, 3 );
                    wallshape.lineTo((data.longueur + 0.1), 3);
                    wallshape.lineTo((data.longueur + 0.1), 0);
                    wallshape.autoClose = true;
                    break
        /* *****Fin forme a estruder pour le mur sans porte ***** */  

        /* *****Début forme a estruder pour le mur avwc porte au centre***** */      
        case false :
                case true :
                      wallshape.moveTo(0, 0);
                      wallshape.lineTo(0, 3 );
                      wallshape.lineTo(data.longueur, 3);
                      wallshape.lineTo(data.longueur, 0);
                      wallshape.lineTo((data.longueur + 0.1)/2 + 0.5, 0);
                      wallshape.lineTo((data.longueur + 0.1)/2 + 0.5, 2);
                      wallshape.lineTo((data.longueur + 0.1)/2 - 0.5, 2);
                      wallshape.lineTo((data.longueur + 0.1)/2 - 0.5, 0);
                      wallshape.autoClose = true;
         ...
         /* *****Ajout d'une primitive a-door ***** */
                          var door = this.el.ensure(".recitdoor", "a-door", {
                            ...
                            'position':{x:-0.2, y: 0, z:(data.longueur +0.1) / 2+ 0.5},
                    ...
                    break
          /* *****fin de Ajout d'une primitive a-door ***** */
                   }

          /* *****Ajout d'un trou pour la fenêtre a-door ***** */
        const whole = new THREE.Path();
                      whole.moveTo(data.longueur / 2- 1, 1);
                      whole.lineTo(data.longueur / 2- 1, 2 );
                      whole.lineTo(data.longueur / 2+ 1, 2 );
                      whole.lineTo(data.longueur / 2+ 1, 1 );
                      whole.moveTo(data.longueur / 2- 15, 1)
                      whole.autoClose = true;

       
                        switch(data.window)
                        {
                          case true:
                            wallshape.holes.push(whole); 
                            this.el.ensure(".recitwindow", "a-window", {
                                          
                              'position':{x:-0.1, y: 1, z:(data.longueur ) / 2+ 1},
                              "rotation" : {x:0, y: 0, z:0}   }  )                   
          ...
          /* *****Fin de l'ajout d'un trou pour la fenêtre a-door ***** */
        ...
        var extrudeSettings = {
          ...
          const murE =  new THREE.ExtrudeGeometry( wallshape, extrudeSettings );
          /* *****Mesh du mur ***** */
          const meshmurE = new THREE.Mesh( murE, material2 );


        /* ***** Construction du mur et porte parBoxBufferGeometry ***** */   
          const boxporte4 = new THREE.BoxBufferGeometry( 1, 2, 0.6 );
          const mur4 = new THREE.BoxBufferGeometry( data.longueur, 3, 0.2 );
          const porte4 = new THREE.Mesh( boxporte4, material2 );
          porte4.rotation.y = THREE.Math.degToRad(( -90))
          porte4.position.z = (data.longueur/2 + 1)
          porte4.position.y = (1)
          const meshmur4 = new THREE.Mesh( mur4, material4 );
          meshmur4.rotation.y = THREE.Math.degToRad(( 90))
          meshmur4.position.z = (data.longueur/2)
          meshmur4.position.y = (1.5)
          //porteV.rotation.y = THREE.Math.degToRad(( -90))
          
          
          /* ***** Soustraction de la prte, du mur  ***** */   
          meshmur4.updateMatrix()
          porte4.updateMatrix()

           const bsp = new CSG()
           const bsp1 = new CSG ()
          bsp1.setFromMesh( porte4 ) 
          bsp.setFromMesh( meshmur4 )
          bsp.subtractOperand(bsp1)
          bsp.toMesh() 
          //mur.add( porte4 );
          mur.add( bsp.toMesh() );
          /* *****Mesh du mur ***** */
